...
+----------------------+------------------------+------------------------------------------+-----------------------------+------------------------+------------------------+
| Universal term       | Claude                 | Codex (app-server)                       | OpenCode                    | Amp                    | Codebuff               |
+----------------------+------------------------+------------------------------------------+-----------------------------+------------------------+------------------------+
| session_id           | n/a (daemon-only)      | n/a (daemon-only)                        | n/a (daemon-only)           | n/a (daemon-only)      | n/a (daemon-only)      |
| native_session_id    | none                   | threadId                                 | sessionID                   | none                   | agentId                |
| item_id              | synthetic              | ThreadItem.id                            | Message.id                  | StreamJSONMessage.id   | synthetic              |
| native_item_id       | none                   | ThreadItem.id                            | Message.id                  | StreamJSONMessage.id   | agentId/toolCallId     |
+----------------------+------------------------+------------------------------------------+-----------------------------+------------------------+------------------------+
...
| Universal term         | Claude                       | Codex (app-server)                         | OpenCode                                | Amp                              | Codebuff                         |
+------------------------+------------------------------+--------------------------------------------+-----------------------------------------+----------------------------------+----------------------------------+
| session.started        | none                         | method=thread/started                      | type=session.created                    | none                             | type=start                       |
| session.ended          | SDKMessage.type=result       | no explicit session end (turn/completed)   | no explicit session end (session.deleted)| type=done                        | type=finish                      |
| turn.started           | synthetic on message send    | method=turn/started                        | type=session.status (busy)              | synthetic on message send        | synthetic on message send        |
| turn.ended             | synthetic after result       | method=turn/completed                      | type=session.idle                       | synthetic on done                | synthetic after finish           |
| message (user)         | SDKMessage.type=user         | item/completed (ThreadItem.type=userMessage)| message.updated (Message.role=user)    | type=message                     | synthetic (input not echoed)     |
| message (assistant)    | SDKMessage.type=assistant    | item/completed (ThreadItem.type=agentMessage)| message.updated (Message.role=assistant)| type=message                  | type=text                        |
| message.delta          | stream_event (partial) or synthetic | method=item/agentMessage/delta      | type=message.part.updated (text-part delta) | synthetic                    | type=text (native)               |
| tool call              | type=tool_use               | method=item/mcpToolCall/progress           | message.part.updated (part.type=tool)   | type=tool_call                   | type=tool_call                   |
| tool result            | user.message.content.tool_result | item/completed (tool result ThreadItem variants) | message.part.updated (part.type=tool, state=completed) | type=tool_result     | type=tool_result                 |
...
| question.requested     | tool_use (AskUserQuestion)  | experimental request_user_input (payload) | type=question.asked                     | none                             | tool_use (ask_user)              |
| question.resolved      | tool_result (AskUserQuestion) | experimental request_user_input (payload) | type=question.replied / question.rejected | none                          | tool_result (ask_user)           |
| error                  | SDKResultMessage.error       | method=error                               | type=session.error (or message error)   | type=error                        | type=error                       |
+------------------------+------------------------------+--------------------------------------------+-----------------------------------------+----------------------------------+----------------------------------+
...
+------------------------------+------------------------+--------------------------+--------------------------------------------------------------+
| item.started (Codebuff)      | Before tool_call/tool_result    | item.started           | Synthetic item.started emitted before item.completed; source=daemon |
| item_id (Codebuff)           | Codebuff provides limited IDs   | item_id                | Generated via counter (codebuff_tool_result_N, etc.)          |
| user message (Codebuff)      | Input prompt not echoed         | item.completed         | Mark source=daemon if user message needs synthesis             |
| question events (Codebuff)   | ask_user tool usage             | question.requested/resolved | Derived from tool_call/tool_result for ask_user tool       |
| subagent tracking (Codebuff) | subagent_start/subagent_finish  | item.started/completed | Status items with label=subagent:{type}                        |
+------------------------------+------------------------+--------------------------+--------------------------------------------------------------+
...
- Codebuff emits native text deltas via type=text events and reasoning deltas via type=reasoning_delta.
...
- subagents: OpenCode subtask parts and Claude Task tool usage are currently normalized into standard message/tool flow (no dedicated subagent fields). Codebuff emits subagent_start/subagent_finish events which are converted to status items with label=subagent:{type}.
